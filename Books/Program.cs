...
app.MapGet("/GetBooksbyID", async (BookContext context, int id) =>
    await context.Books.FindAsync(id) is Book book ?
        Results.Ok(book) :
        Results.NotFound("Sorry, book not found. :/"));

app.MapPost("/Add Book", async (BookContext context, Book book) =>
{
...
app.MapPut("Update Book", async (BookContext context, Book book, int Id) =>
{
...
app.MapDelete("/Delete Book", async (BookContext context, int Id) =>
{
    var dbBook = await context.Books.FindAsync(Id);
